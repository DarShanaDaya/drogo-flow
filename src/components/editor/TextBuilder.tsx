    ...
    <div className="flex flex-col h-full border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900">
      <div className="flex items-center justify-between p-2.5 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
        <h3 className="font-semibold text-xs flex items-center gap-1.5">
          ...
          <span className="ml-1 px-1.5 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded text-[10px] font-normal">{code.split('\n').length} lines</span>
        </h3>
      ...
      <div className="flex-1 relative">
        ...
        />
        <div className="absolute bottom-2 right-2 text-[10px] text-zinc-500 bg-zinc-900/80 backdrop-blur px-2 py-1 rounded">
          {code.length} chars
        </div>
      </div>

      <div className="p-2.5 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 max-h-[200px] overflow-auto">
        ...
        <div className="flex flex-wrap gap-1">
          {SNIPPETS.map(s => (
            <button
              ...
              className="px-2 py-1 text-[11px] rounded-full bg-zinc-100 hover:bg-zinc-900 hover:text-white dark:bg-zinc-800 dark:hover:bg-white dark:hover:text-zinc-900 transition-colors border border-zinc-200 dark:border-zinc-700"
            >
              {s.label}
            </button>
          ))}
        ...
        <div className="mt-3 flex gap-3 text-[11px] text-zinc-500">
          <a href="https://mermaid.js.org/syntax/flowchart.html" target="_blank" className="text-blue-600 hover:underline flex items-center gap-1">📚 Docs</a>
          <a href="/docs" target="_blank" className="hover:underline">📖 Our Docs</a>
          <span>Ctrl+S save • Ctrl+B sidebar</span>
        </div>
      </div>
    </div>
  );
